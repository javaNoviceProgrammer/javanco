	...
	private void initPaintedComponent() {
		paintedComponent = new JComponent() {
			...
			public void paintComponent(Graphics g) {
				dim = painter.paintToGraphics(g, this.getSize(), data, dataI, choosenCmap, grid);
				if (data != null) {
					xmult = (int)(dim.getWidth() / data.length);
					ymult = (int)(dim.getHeight() / data[0].length);
				}
			}
		};
				...
				save.addActionListener(new ActionListener() {	
					...
					public void actionPerformed(ActionEvent e) {
						if (dim != null) {
							...
							painter.paintToGraphics(g, dim, data, dataI, choosenCmap, grid);								try {
