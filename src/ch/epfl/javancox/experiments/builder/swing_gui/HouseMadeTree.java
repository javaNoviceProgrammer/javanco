...
public class HouseMadeTree<X> extends JScrollPane implements TreeModelUIManager, MouseWheelListener {
	...
	class LocalPanel extends JPanel {
		...
		public void updateSize(Dimension d) {
			if (d.width != getSize().getWidth() || d.height != getSize().getHeight()) {
				super.setSize(d);
				rebuild();
			}
		}
	...
	/*	int width;
		if (getParent() != null) {
			width = Math.max(mainPanel.getSize().width, max);
		} else {
			width = Math.max(600, max);
		}*/
        

		
	//	mainPanel.updateSize(new Dimension(width, lines*yOffset));
