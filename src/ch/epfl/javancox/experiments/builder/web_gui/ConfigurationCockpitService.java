...
public class ConfigurationCockpitService extends AbstractService {
	...
	protected void process(Request req, Response resp) throws Exception {
		...
		try {
			...
			String st = cont.toString().replace("#GOING_TO_BE_REPLACED#", getLinesAndMenu(getSessionObject(session)).toString());
			ow.write(st);
			ow.flush();
			out.close();
		} catch (Exception e) {
	...
	/*	ExperimentExecutionManager executionManager = new ExperimentExecutionManager() {
			@Override
	...
	private void appendLine(AbstractChooseNode node, StringBuilder sb, SessionObject session) {
		...
		if (node instanceof ClassChooseNode) {
			ClassChooseNode cNode = (ClassChooseNode)node;
