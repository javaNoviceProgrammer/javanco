...
import ch.epfl.general_libraries.charts.CustomLogAxis;
...
import ch.epfl.javancox.results_manager.display.gui.MatlabPlot.MATLABPlotMap;
...
public class XYChartProvider extends AbstractChartProvider {
	...
	private ChartContainer createScalarChart(DataRetrievalOptions options, AdvancedDataRetriever retriever) {
        ...
        ((NumberAxis)chart.getChart().getXYPlot().getDomainAxis()).setAutoRangeIncludesZero(false);
        ((NumberAxis)chart.getChart().getXYPlot().getRangeAxis()).setAutoRangeIncludesZero(false);
...
//		expIDs.clear();
		...
		for( DataSeries cv : dataSeries) {
			...
			for( DataSeries.DataSeriesSorter.XEntry xe : entries ) {
				...
				if( xeValues.size() > 1 && isWithLines ) {
					...
					if( !options.is95 ) {
						all = StatFunctions.getAll(xe.values, (float) options.confInt / 100);
					} else {
						all = StatFunctions.getAll95(xe.values, (float) options.confInt / 100);
					}
	...
	public void createMatlabData(ComplexDisplayPanel panel) throws Exception {
		...
		java.io.FileWriter fw = new java.io.FileWriter(file + ".m");
		//******************************************
...
//			fw.append(str);
		...
		for( int i = 0; i < totalColors; i++ ) {
			int colorId;
			if (totalColors > 63) {
				colorId = (int)(((double)i/(double)totalColors)*63);
			} else {
				colorId = i;
			}

			String pStr = "p" + i + " = plot(";
			pStr += "xVec_" + i + ", " + "yVec_" + i  +",'LineWidth',2,'Color',cmap(" + (colorId+1) +",:)";
			fw.append(pStr);
			if (isWithoutLines) {
				pStr = ",'LineStyle','none','Marker','s','MarkerFaceColor',cmap(" + (colorId+1) + ",:),'MarkerEdgeColor',cmap(" + (colorId+1) + ",:));\r\n";
			} else {
				pStr = ");\r\n";
			}
	...
	public void createExcelData(AdvancedDataRetriever retriever) throws Exception {
		...
		if( nbParams > 0 ) {
			...
			fileChooser.showSaveDialog(null) ;
			String file = fileChooser.getSelectedFile().getAbsolutePath() ;
			java.io.FileWriter fw = new java.io.FileWriter(file + ".csv");
			//******************************************
